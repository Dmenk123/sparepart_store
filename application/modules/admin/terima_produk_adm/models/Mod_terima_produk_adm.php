...
class Mod_terima_produk_adm extends CI_Model
{
	...
	private function _get_datatable_terima_query($term='') //term is value of $_REQUEST['search']
	{
		$column = array(
				'tbl_trans_masuk.id_trans_masuk',
				'tbl_user.fname_user',
		...
		$this->db->select('
				...
				tbl_supplier.nama_supplier,
				tbl_trans_masuk.tgl_trans_masuk,
				COUNT(tbl_trans_masuk_detail.id_produk) AS jml
			');
		...
		$this->db->group_by('tbl_trans_masuk_detail.id_trans_masuk');
		$i = 0;
	...
	function get_datatable_penerimaan()
	{
		...
		$this->_get_datatable_terima_query($term);
	...
	function count_filtered_penerimaan()
	{
		$term = $_REQUEST['search']['value'];
		$this->_get_datatable_terima_query($term);
		$query = $this->db->get();
	...
	public function count_all_penerimaan()
	{
		$this->db->from('tbl_trans_masuk');
	...
	public function get_data_masuk_header($id_trans_masuk)
	{
		$this->db->select('
			tbl_trans_masuk.id_trans_masuk,
			tbl_user.fname_user,
			...
			tbl_trans_masuk.tgl_trans_masuk,
			tbl_trans_masuk_detail.id_trans_order
		');
		$this->db->from('tbl_trans_masuk');
		$this->db->join('tbl_user', 'tbl_trans_masuk.id_user = tbl_user.id_user','left');
		$this->db->join('tbl_supplier', 'tbl_trans_masuk.id_supplier = tbl_supplier.id_supplier','left');
		$this->db->join('tbl_trans_masuk_detail', 'tbl_trans_masuk_detail.id_trans_masuk = tbl_trans_masuk.id_trans_masuk','left');
        $this->db->where('tbl_trans_masuk.id_trans_masuk', $id_trans_masuk);
        $this->db->group_by('tbl_trans_masuk_detail.id_trans_order');
	...
	public function get_data_masuk_detail($id_trans_masuk)
	{
		$this->db->select('tbl_trans_masuk_detail.id_trans_masuk,
						   tbl_produk.nama_produk,
						   tbl_trans_masuk_detail.id_trans_masuk_detail,
						   tbl_trans_masuk_detail.id_trans_order_detail,
						   tbl_trans_masuk_detail.id_trans_order,
						   ...
						   tbl_trans_masuk_detail.timestamp');
