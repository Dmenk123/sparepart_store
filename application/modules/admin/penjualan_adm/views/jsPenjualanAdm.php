...
$(document).ready(function() {
   ...
   // tabel trans masuk
	table = $('#tableConfirmJual').DataTable({
      ...
      "order":[], //initial no order 
		...
		"ajax": {
			"url": "<?php echo site_url('penjualan_adm/list_penjualan') ?>",
			"type": "POST" 
		},
   ...
   $(document).on('click', '.btn_edit_status', function(){
      if(confirm('Apakah anda yakin ubah status Penjualan ini ?'))
      {
         ...
         $.ajax({
            url : "<?php echo site_url('penjualan_adm/edit_status_penjualan')?>/"+id,
            type: "POST",
            dataType: "JSON",
            data : {status : status},
...
function editConfirmPenjualan(id)
{
   ...
   $.ajax({
      url : "<?php echo site_url('penjualan_adm/get_konfirmasi_penjualan/')?>" + id,
      type: "GET",
      dataType: "JSON",
      success: function(data)
      {
         ...
         $('[name="fieldIdCustomer"]').val(data.data_header[0].id_user);
