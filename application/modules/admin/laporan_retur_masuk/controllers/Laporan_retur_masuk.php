...
class Laporan_retur_masuk extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Mod_lap_ret_masuk','m_retmsk');
	...
	public function laporan_retur_masuk_detail()
	{
		...
		$query = $this->m_retmsk->get_detail_lap_retur_masuk($tanggal_awal, $tanggal_akhir);
	...
	public function cetak_laporan_retur_masuk($tglAwal= 0, $tglAkhir= 0)
	{
		$this->load->library('Pdf_gen');
		$id_user = $this->session->userdata('id_user');
		$query = $this->m_retmsk->get_detail_lap_retur_masuk($tglAwal, $tglAkhir);
	    ...
	    $filename = 'laporan_penerimaan_retur_'.time();
